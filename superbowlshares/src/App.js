...
const App = () => {
  ...
  const [ledgerData, setLedgerData] = useState([]); // State for the ledger table
    ...
    const fetchLedgerData = async () => {
      try {
        const backendUrl = window.location.hostname.includes("ngrok")
          ? 'https://100.26.132.85:3001/api/ledger'
  ...
  useEffect(() => {
    localStorage.setItem('sharesData', JSON.stringify(sharesData));
  }, [sharesData]);
  ...
  // Effect to reaggregate the ledger to populate other tabs
  useEffect(() => {
  ...
  const ledgerColumns = React.useMemo(
    () => [
	  ...
	  /* {
        ...
        accessor: 'actions',
        Cell: ({ row }) => (
          <button className="delete-button" onClick={() => handleLedgerDeleteRow(row.id)}>Delete</button>
        ),
      }, */
    ],
    []
  ...
  const aggregateLedgerData = (ledgerData) => {
	...
	setTotalMoney(calculateTotalMoney(moneyData));
